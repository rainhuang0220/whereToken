...
describe('theme is a page, not a home strip', () => {
  ...
  it('collapses with a dedicated leave Flip, not enter timeline.reverse', () => {
    const page = readFileSync(join(SRC_ROOT, 'pages/Themes.vue'), 'utf8')
    const css = readFileSync(join(SRC_ROOT, 'styles.css'), 'utf8')
    const closeStart = page.indexOf('async function closeCard')
    const closeEnd = page.indexOf('function onSlabClick')
    expect(closeStart).toBeGreaterThan(0)
    expect(closeEnd).toBeGreaterThan(closeStart)
    const close = page.slice(closeStart, closeEnd)
    expect(close).not.toMatch(/\.reverse\s*\(/)
    expect(close).not.toMatch(/canReverse/)
    expect(close).toMatch(/fadeInnerPreview/)
    expect(close).toMatch(/captureFlip\(\s*hero/)
    expect(close).toMatch(/restoreGallery/)
    expect(close).toMatch(/afterPaint/)
    expect(close.indexOf('fadeInnerPreview')).toBeLessThan(close.indexOf('captureFlip'))
    expect(close.indexOf('captureFlip')).toBeLessThan(close.lastIndexOf('revealMock.value = false'))
    expect(close.indexOf('captureFlip')).toBeLessThan(close.lastIndexOf("openId.value = null"))
    expect(close).toMatch(
      /restoreGallery\(\{[\s\S]*onSettled\(\)\s*\{[\s\S]*applyTheme\(\s*committed\.value/,
    )
    expect(page).not.toMatch(/startViewTransition/)
    expect(css).toMatch(/\.glaze-hall\.is-leaving/)
    expect(css).toMatch(/\.glaze-hall\.is-leaving[^{]*\.glaze-slab:hover[^{]*\{[^}]*transform:\s*none/)
  })
})
