...
func TestHelpTextMentionsPrivacyAndInstall(t *testing.T) {
	...
	for _, want := range []string{"go install", "curl -fsSL", "install.sh", "install.ps1", "ExecutionPolicy Bypass", "JWT", "127.0.0.1", "EXIT CODES", "--tool", "--today", "EXAMPLES", "NO_COLOR", "WHERETOKEN_HOME", "--quiet", "--width", "truncating names", "--offline", "FORCE_COLOR", "--today --cursor", "--today --kimi", "--today --grok", "schema 1", "per-tool", "--model=k3", "cli-json.schema.json", "[flags] sources", "--version", "--port", "./Formula/wheretoken.rb", "unsigned", "brew tap rainhuang0220/wheretoken", "刷新", "xai", "CLI table", "observatory JSON", "not schema 1", "brew --HEAD", "scan is a different dump", "[flags] update", "[flags] uninstall"} {
