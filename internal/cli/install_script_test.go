...
func TestManPageMentionsJSONSchema(t *testing.T) {
	...
	s := string(body)
	for _, want := range []string{"cli-json.schema.json", "JWT", "127.0.0.1", "--offline", "--width", "--version", "install.sh", "go install", "--port", "--today --kimi", "unsigned", "brew tap rainhuang0220/wheretoken", "COLUMNS", "FORCE_COLOR", "刷新", "xai", "Community Rank", "DO_NOT_TRACK", "Does not upload", "估价", "uploaded", "not $0", "not #0"} {
		if !strings.Contains(s, want) {
