...
`wheretoken` with no command stays a local report and does not upload. `profile refresh` uploads at most one sanitized snapshot when the local all-time total has moved by at least 10,000 raw tokens, or the local calendar date has changed and the new total is available and at least as large as the last accepted total. A publish also waits until one hour has passed since the hosted envelope's `freshness.updated_at`. A date change does not skip that hour. A smaller total is not published. Missing totals stay unavailable and are not treated as zero. `profile refresh on` only persists the switch that `watch`, status, and doctor read. The bare command is an explicit one-shot even when the switch is off. `watch` runs only while the switch is on. The snapshot stays `manual_publish` with `live_sync: false`.
