...
The production flow for the committed bundle is private ledger → local release binary → sanitized bundle → explicit Git commit. GitHub Actions never scans HOME. The hosted projection is separate: `wheretoken profile refresh` (and a paired `wheretoken sync`) PUT the sanitized snapshot. The switch defaults to off. The install scripts do not enable it, and a plain `wheretoken` report does not upload.

`wheretoken profile refresh on` writes `schema_version` and `enabled` in `profile-refresh.json` beside `community.json`, flushes that file, and only then, on macOS, installs a user launchd agent. `wheretoken profile refresh off` sets the switch off and uninstalls that agent. Linux and Windows do not install a background agent; `wheretoken profile refresh watch` is the foreground loop. The bare `profile refresh` command is one explicit PUT even when the switch is off, and it is not the program launchd runs.

`profile refresh` uploads at most one sanitized snapshot when the local all-time total has moved by at least 10,000 raw tokens, or the local calendar date is strictly later and the new total is available and at least as large as the last accepted total. A publish also waits until one hour has passed since the hosted envelope's `freshness.updated_at`. A missing `updated_at` does not count as an elapsed hour. A date change does not skip that hour. An earlier local date is not a rollover. A smaller total is not published. Missing totals stay unavailable and are not treated as zero. `watch` re-reads the switch every 15 minutes and exits when it is off. A powered-off machine does not publish; catch-up is the next wake of that loop, or RunAtLoad after a graphical login. An SSH session with no GUI login does not start the agent, and `profile refresh status` can show `scheduler=dead`. The snapshot stays `manual_publish` with `live_sync: false`.

That PUT updates the interactive hosted snapshot. The GitHub README is a separate consumer. The first README write uses the verified palette, or newsprint when none has been verified. Same-day README materialization still waits 30 minutes, then 100,000 tokens or 6 hours. A strictly later `as_of_date` materializes immediately. An earlier date does not. A failed README write leaves the projection in place and is retried by the hosted process without another PUT.
