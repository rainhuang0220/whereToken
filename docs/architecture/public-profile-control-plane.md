...
4. `wheretoken profile refresh` is separate from the default report. The report does not upload. The command does not upload the daily batch. `profile refresh on` records only an enabled flag in `profile-refresh.json` next to `community.json`; that flag arms `profile refresh watch`, status, and doctor. The bare command runs one decide-and-PUT even while the switch is off. `watch` runs only while the switch is on and repeats the check in the foreground every 15 minutes. The watermark is the hosted envelope (`as_of_date`, all-time total, `freshness.updated_at`), not the scan index. A smaller all-time total than the last accepted snapshot is not published, including when the local date changed.
