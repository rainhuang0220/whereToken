...
Normals are wrapped central differences of that height, with derivative scale 2.4 on the static fallback. Roughness stays high (about 0.88, with a small formation term). Lighting is evaluated in linear light: a broad key from the upper left, Oren–Nayar diffuse, and a very small GGX term, then encoded to sRGB. The sheet is retinted toward editorial near-white. The same image remains the static fallback. The live Newsprint page also loads `newsprint-height.jpg` into a WebGL2 shader (`assets/paper.js`) that rebuilds normals and shades the sheet in real time. A broad window, not a spotlight, makes the near side of the sheet a little brighter as the light moves. Pointer movement on a fine pointer eases that light; reduced motion and touch keep a fixed light. Dark mode draws that light sheet inside a dark surround.
